...
def get_dir_path():
    ...
    #print 'refrence文件路径：',refrencePath
    for i in refrencePath:
        version_info.append(i.split('/')[:-2])
    #print '拆分refrence文件目录路径',version_info
...
def show_path_message():
    ...
    if len(path)>0:
        ...
        cmds.button( label='Close', command=('cmds.deleteUI(\"' + window + '\", window=True)'),recomputeSize = 1 ,align ='right')
